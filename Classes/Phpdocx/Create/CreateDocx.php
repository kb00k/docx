...
use Phpdocx\Elements\CreateOnlineVideo;
...
use Phpdocx\Utilities\DOCXStructureFromStream;
use Phpdocx\Utilities\DOCXStructureTemplate;
use Phpdocx\Utilities\Fonts;
...
class CreateDocx extends CreateDocument
{
    ...
    public function addOnlineVideo($src, $options = array())
    {
        ...
        if ($this instanceof CreateDocx && isset($this->target)) {
        ...
        // file image
        if (isset($options['image'])) {
            if (file_exists($options['image']) == 'true') {
     ...
     *      'styleEmbeding' (string) Regular (default), Bold, Italic, BoldItalic
     */
    public function embedFont($fontSource, $fontName, $options = array())
    {
        if (!file_exists(dirname(__FILE__) . '/../Utilities/Fonts.php')) {
            PhpdocxLogger::logger('This method is not available for your license.', 'fatal');
        }
        ...
        if (!isset($options['styleEmbeding'])) {
            $options['styleEmbeding'] = 'embedRegular';
        } else {
            // concat embed as prefix
            $options['styleEmbeding'] = 'embed' . ucfirst($options['styleEmbeding']);
        }
        ...
        if (file_exists($fontSource)) {
            ...
            $idFont = rand(999, 99999999);

            // check if the same font name already exists. If true, add the new embeded font to the w:font tag only if the styleEmbeding doesn't exist
            ...
            if ($fontQuery->length > 0) {
                // add the new embed font to the existing w:font tag if the styleEmbeding doesn't exist
                $fontQueryEmbed = $fontQuery->item(0)->getElementsByTagNameNS('http://schemas.openxmlformats.org/wordprocessingml/2006/main', $options['styleEmbeding']);
                if ($fontQueryEmbed->length == 0) {
                    // font entry to be added
                    $fontEntry = '<w:' . $options['styleEmbeding'] . ' r:id="rId' . $idFont . '" w:fontKey="' . $odttfFont['guid'] . '"/>';
            ...
            PhpdocxLogger::logger('File does not exist.', 'fatal');
        }
    }
    ...
    public function embedHTML($html = '<html><body></body></html>', $options = array())
    {
        ...
        if (is_array($sFinalDocX[4])) {
            foreach ($sFinalDocX[4] as $value) {
                ...
                if (file_exists(dirname(__FILE__) . '/../Transform/HTMLExtended.php') && $options['useHTMLExtended']) {
     ...
     * 
     * @access public
     * @param string $document path or DOCXStructure of the document that includes the contents to be imported
     * @param array $referenceNode
     * Keys and values:
     *     'type' (string) can be * (all, default value), bookmark, break, chart, endnote (content reference), footnote (content reference), image, list, paragraph (also for links and lists), section, shape, table
     *     'contains' (string) for bookmark, list, paragraph (text, link), shape
    ...
    {
        if (file_exists(dirname(__FILE__) . '/../Utilities/DOCXStructureTemplate.php') && $document instanceof DOCXStructure) {
            // use in-memory DOCX
            $documentInMemory = $document;
        } else {
            // use a file, load it as in-memory DOCX
            $docxStructureStream = new DOCXStructureFromStream();
            $documentInMemory = $docxStructureStream->generateDOCXStructure($document);
        }
        ...
        if ($this instanceof CreateDocx && !($this instanceof CreateDocxFromTemplate)) {
    ...
    public function importListStyle($path, $id, $name)
    {
        ...
        $listId = rand(999, 9999);
        $originalAbstractNumId = rand(999, 9999);
            ...
            if ($elementsMR->length > 0) {
                foreach ($elementsMR as $elementMR) {
                    ...
                    if ($elementColor->length == 0) {
                        ...
                        $elementColorItem = $elementRPRSZ->item(0);
                    }
