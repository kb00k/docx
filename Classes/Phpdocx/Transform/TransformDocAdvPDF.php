...
/**
 * Transform DOCX to PDF using native PHP classes and TCPDF
 *
 * @category   Phpdocx
 * @package    transform
...
class TransformDocAdvPDF
{
    ...
    public function transform($target, $options = array())
    {
        ...
        if (file_exists(dirname(__FILE__) . '/../Utilities/ZipStream.php') && (\Phpdocx\Create\CreateDocx::$streamMode === true || (isset($options['stream']) && $options['stream']))) {
                    ...
                    if (isset($pprStyle->tagName)) {
                        switch ($pprStyle->tagName) {
                            ...
                            case 'w:shd':
                                ...
                                break;
                            case 'w:spacing':
                                // prevent adding spacing from the default styles to avoid wrong spacings
                                if ($pprStyle->hasAttribute('w:line') && !$defaultStyles) {
    ...
    protected function transformW_DRAWING($childNode, $nodeClass)
    {
        ...
        if ($elementABlip) {
            ...
            if ($elementWPPositionH->length > 0) {
                ...
                // check if the parent w:p has an alignment
                if ($childNode->parentNode->parentNode && $childNode->parentNode->parentNode->tagName == 'w:p') {
                    $jcNode = $childNode->parentNode->parentNode->getElementsByTagNameNS('http://schemas.openxmlformats.org/wordprocessingml/2006/main', 'jc');
                    if ($jcNode->length > 0 && $jcNode->item(0)->hasAttribute('w:val')) {
                        $alignMode = true;
                        if ($jcNode->item(0)->getAttribute('w:val') == 'right') {
                            $palign = 'R';
                        } elseif ($jcNode->item(0)->getAttribute('w:val') == 'left') {
                            $palign = 'L';
                        } elseif ($jcNode->item(0)->getAttribute('w:val') == 'center') {
                            $palign = 'C';
                        }
                    }
                }
