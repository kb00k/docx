...
class HTML2WordML
{
    ...
    public function render($html, $options, $docx = null)
    {
        ...
        if (isset($options['useHTMLExtended'])) {
            $this->htmlExtended = $options['useHTMLExtended'];
        }
        ...
        if (file_exists(dirname(__FILE__) . '/HTMLExtended.php') && $this->htmlExtended) {
    ...
    private function renderDompdf($html, $isFile = false, $filter = '*', $parseDivs = false, $baseURL = '', $disableWrapValue = false)
    {
        ...
        $dompdf = new \Phpdocx\Libs\PARSERHTML();
    ...
    private function _render($nodo, $depth = 0)
    {
        ...
        switch ($nodo['nodeName']) {
                ...
                if ($this->customListStyles) {
                    ...
                    if ($listLevel == 0) {
                        ...
                        if (!empty($currentListStyle)) {
                            self::$currentCustomList = rand(999, 9999);
                        ...
                        if (file_exists(dirname(__FILE__) . '/HTMLExtended.php') && $this->htmlExtended) {
                            ...
                            if (isset($nodo['attributes']) && isset($nodo['attributes']['class'])) {
                                ...
                                if (!is_null($currentListStyle)) {
                                    self::$currentCustomListLvlOverride = rand(999, 9999);
            ...
            case 'a':
                ...
                break;
            case 'span':
                if (isset($nodo['attributes']) && isset($nodo['attributes']['class']) && isset($nodo['attributes']['class'][0])) {
                    $this->spanStyle = $nodo['attributes']['class'][0];
                }
                if (isset($nodo['attributes']) && isset($nodo['attributes']['data-rpr'])) {
                    $this->rprStyle = $nodo['attributes']['data-rpr'];
                }
                $this->wordML .= $this->closePreviousTags($depth, $nodo['nodeName']);
                self::$openTags[$depth] = $nodo['nodeName'];
                break;
            ...
            case '#text':
                ...
                if (self::$openSelect) {
                    ...
                    $this->wordML .= '</w:r>';
                    if (self::$isTextCaption == true) {
                        $this->wordML .= '<w:fldSimple w:instr=" SEQ Figure \* ARABIC "><w:r><w:rPr><w:noProof/></w:rPr><w:t></w:t></w:r></w:fldSimple>';
            ...
            case 'img':
                ...
                if (isset($properties['float']) && ($properties['float'] == 'left' || $properties['float'] == 'right')) {
                    ...
                    $this->wordML .= '<wp:docPr id="' . rand(99999, 99999999) . '" name="' . $tempName . '" descr="' . rawurlencode($descr) . '" /><wp:cNvGraphicFramePr><a:graphicFrameLocks xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" noChangeAspect="1" /></wp:cNvGraphicFramePr>';
            ...
            case 'hr':
                ...
                $widthHR = 'style="width:0;height:1.5pt"';
                if (isset($properties['width']) && $properties['width'] != 'auto') {
                    $widthHRPT = $this->length_in_pt($properties['width']) . 'pt';

                    $widthHR = 'style="width:'.$widthHRPT.';height:1.5pt" o:hrpct="0"';
                }
                if (self::$openPs) {
                    $this->wordML .= '<w:r><w:pict><v:rect id="_x0000_i1026" '.$widthHR.' o:hralign="center" o:hrstd="t" o:hr="t" o:hrnoshade="t" fillcolor="'.$colorHR.'" stroked="f" /></w:pict></w:r>';
                } else {
                    $this->wordML .= '<w:p><w:r><w:pict><v:rect id="_x0000_i1026" '.$widthHR.' o:hralign="center" o:hrstd="t" o:hr="t" o:hrnoshade="t" fillcolor="'.$colorHR.'" stroked="f" /></w:pict></w:r></w:p>';
                }
                break;
            case 'input':
                $this->wordML .= $this->closePreviousTags($depth, $nodo['nodeName']);
                        ...
                        if ($extendedTags[$nodo['nodeName']] == 'addHeader' || $extendedTags[$nodo['nodeName']] == 'addFooter') {
                            ...
                            if (!empty($wordFragmentHtmlContent)) {
                                ...
                                if ($extendedTags[$nodo['nodeName']] == 'addHeader') {
                                    $wordFragmentContent = new \Phpdocx\Elements\WordFragment($this->docx, $attributesContent['type'] . 'Header');
                                ...
                                if ($extendedTags[$nodo['nodeName']] == 'addFooter') {
                                    $wordFragmentContent = new \Phpdocx\Elements\WordFragment($this->docx, $attributesContent['type'] . 'Footer');
                                    $wordFragmentContent->embedHTML($wordFragmentHtmlContent, array('useHTMLExtended' => true));
                            ...
                            $wordFragmentContent = new \Phpdocx\Elements\WordFragment($this->docx, $attributesContent['target']);
                            ...
                            $transformedContent = $htmlExtendedContent->getContent($nodo, $extendedTags[$nodo['nodeName']], self::$openPs, $attributesContent);
    ...
    private function generateRPr($properties, $level = '', $attributes = array(), $nodeName = false)
    {
        ...
        if (@$properties['font_weight'] == 'bold' || @$properties['font_weight'] == 'bolder' || @$properties['font_weight'] == '700' || @$properties['font_weight'] == '800' || @$properties['font_weight'] == '900') {
            $stringRPr .='<w:b /><w:bCs />';
        }
        ...
        if (isset($properties['vertical_align']) && $properties['vertical_align'] != 'baseline') {
            ...
            } else if ($properties['vertical_align'] == 'supper') {
        ...
        if (file_exists(dirname(__FILE__) . '/HTMLExtended.php') && $this->htmlExtended && $this->rprStyle != null) {
            $stringRPr .= html_entity_decode($this->rprStyle);
            $this->rprStyle = null;
        }
    ...
    private function generateHTMLExtendedNote($node, $nodeName, $attributesContent)
    {
        ...
        foreach ($contents as $content) {
            if ($content->tagName == $textDocumentScope) {
                ...
                $wordFragmentTextContent = new \Phpdocx\Elements\WordFragment($this->docx, $target);
                $wordFragmentHtmlContent = '';
                if (!empty($this->CSSdocument)) {
                ...
                $wordFragmentContent = new \Phpdocx\Elements\WordFragment($this->docx, $target);
                ...
                $wordFragmentTextContent = new \Phpdocx\Elements\WordFragment($this->docx);
                $wordFragmentHtmlContent = '';
                if (!empty($this->CSSdocument)) {
